...
void University::add_student(const char *const name, const char *const last_name,
                             const unsigned int id, const char *const password, const bool notify) {
...
void University::flush() {
    ...
    if (file) {
        ...
        for (std::map<const unsigned int, Student *>::const_iterator citer =
                 student_records.begin();
             citer != student_records.end(); ++citer) {
            file << citer->second->name << " " << citer->second->last_name << " "
                 << citer->second->id << " " << citer->second->password << std::endl;
        }
    }
}
...
void University::close() {
    ...
    for (std::map<const unsigned int, Student *>::iterator iter = student_records.begin();
        ...
        iter->second->name = 0;
        delete[] iter->second->last_name;
        iter->second->last_name = 0;
...
void University::print_records() {
    ...
    for (std::map<const unsigned int, Student *>::const_iterator citer = student_records.begin();
        ...
        std::cout << citer->second->name << " " << citer->second->last_name << " "
...
void University::exmatriculate(const unsigned int id) {
    ...
    if (iter != student_records.end()) {
        std::cout << "Examtriculating Student: " << iter->second->name << " "
                  << iter->second->last_name << std::endl;
        ...
        std::cout << "No Student with id " << id << " imatriculated at B-TU" << std::endl;
    }
}
...
void University::request_exmatriculation(const unsigned int id, const char *const password) {
    ...
    if (citer != student_records.end()) {
        ...
        std::cout << "No Student with id " << id << " imatriculated at " << name << std::endl;
    }
}

void University::notifyStudentOnExmatriculation(const unsigned int id) {
    // TODO: Print, sign, package, stamp and send official letter to affected
    // students NOT IMPLEMENTED YET

    std::cout << "We have sent out an exmatriculation notification to student " << id
              << "\nThank you for using B-TU Student Manager." << std::endl;
}
...
void University::notifyStudentOnImatriculation(const unsigned int id) {
    ...
    std::cout << "We have sent out an imatriculation letter to student " << id
              << "\nThank you for using B-TU Student Manager." << std::endl;
}

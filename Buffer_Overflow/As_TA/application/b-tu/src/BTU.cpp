...
int main(int argc, char **argv) {
    if (argc < 2) {
        std::cout << "usage (Adding a new student): btu add Name Last_Name ID Password\n"
                  ...
                  << "usage (Printing all records): btu print" << std::endl;
        std::cout << "Please note: Students can only be exmatriculated using the same password "
                     "that was used to imatriculate them."
                  << std::endl;
        return 0;
    }
    ...
    strcpy(imprint, "This is B-TU Student management System V1.0"

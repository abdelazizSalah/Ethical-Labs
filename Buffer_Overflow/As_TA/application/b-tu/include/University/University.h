...
class University {
  public:
     ...
     */
    University(const std::string name) : name(name), db_path(name + ".db") { initialize(); }
     ...
     */
    void add_student(const char *const name, const char *const last_name, const unsigned int id,
                     const char *const password, const bool notify = false);
